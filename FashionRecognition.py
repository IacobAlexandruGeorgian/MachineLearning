...
def prepareData(trainX, trainY, testX, testY):
    ...
    trainX = trainX.reshape(trainX.shape[0],28,28,1).astype('float')
    testX = testX.reshape(testX.shape[0],28,28,1).astype('float')
...
def defineModel(input_shape, num_classes):
    ...
    model.add(Conv2D(64, (3, 3), input_shape=(28, 28, 1), padding='same', activation='relu', kernel_initializer='he_uniform'))
...
def main():
    ...
    #TODO - Exercise 1:
    #for i in range(9):
    #    cv2.imshow('Image number {}'.format(i), trainX[i])
    #cv2.waitKey(0)
    ...
    # TODO - Exercise 8 - use image
    image = cv2.imread("sample_image.png")
    imagegray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    imagegray = cv2.resize(imagegray, (28,28))
    cv2.imshow("img:", imagegray)
    cv2.waitKey(0)
    imagegray = imagegray.reshape(1, 28, 28, 1).astype('float')
    imagegray = imagegray / 255


    #TODO - Exercise 8 - use model on image
    model = load_model('Fashion_MNIST_model.h5')
    clasa =model.predict_classes(imagegray)
    print("Clasa: {}".format(clasa))
    #np.argmax(model.predict(imagegray), axis=-1)
